      ...
      m_filterObjectSize(filterObjectSize)
{
    ...
    Point_t pt(m_predictionPoint.x, m_predictionPoint.y); // 这个地方要注意，放进去的点有点问题。
                    ...
                    int trajLen)
{
    ...
    if (dataCorrect)	// 有匹配，就push_back一对
    {
		...
		// 检测目标是否静止了
        CheckStatic(trajLen, currFrame, region);	// 不进行静止检测，因为静止的就根本不会检测出来。
    }
    else				// 没匹配，就push_back预测点进去
    {
...
track_t CTrack::CalcMahalanobisDist(const cv::RotatedRect& rrect) const
{
	...
	if (!res1.empty() && !res2.empty()) {
		...
		cv::Mat predictPoint = (cv::Mat_<track_t>(2, 1) << res2.data[0], res2.data[1]);		// 预测
		MahaDist = cv::Mahalanobis(measurePoint, predictPoint, res1);				// res2为残差协方差
...
{
	// m_settings.m_useAbandonedDetection = true; trajLen有一个数值
    if (!trajLen || static_cast<int>(m_trace.size()) < trajLen)			// 若当前轨迹长度<阈值 || 没有轨迹
    {
        m_isStatic = false;												// 该目标没有静止，静止张数为0，静止帧
        m_staticFrames = 0;
        m_staticFrame = cv::Mat();
    }
    else // 有长度、且长度>trajLen，轨迹长度大于静止判断阈值，开始判断是否静止
    {
        track_t kx = 0;
        track_t bx = 0;
        track_t ky = 0;
        track_t by = 0;
		// 获取线性回归参数，获取当前目标的速度，若速度小于阈值，则目标静止
        ...
        {
            if (!m_isStatic)		// 原来是运动的，记录静止时的图像，记录静止时的框
            {
                ...
                cv::Mat img = m_staticFrame.clone();
                        ...
                        cv::Mat currFrame)
{
    ...
    case tracking::TrackLDES:	// 暂时不用这个
        break;
    }
...
{
    m_kalman.GetPointPrediction();	// 调用predict并用predict更新m_lastPointResult

    m_predictionPoint = m_kalman.Update(pt, dataCorrect);	// correct
