...
track_t CTrack::CalcMahalanobisDist(const cv::RotatedRect& rrect) const
{
	...
	if (!res1.empty() && !predictPoint.empty()) {
		...
		if (predictPoint.rows == 2)			// PointUpdate
			measurePoint = (cv::Mat_<track_t>(2, 1) << rrect.center.x, rrect.center.y); // 观测
		else
			measurePoint = (cv::Mat_<track_t>(4, 1) << rrect.center.x, rrect.center.y, rrect.size.width, rrect.size.height); // 观测
...
{
    //m_kalman.GetPointPrediction();							// 更新预测

    m_predictionPoint = m_kalman.Update(pt, dataCorrect);	// 用观测修正预测

    if (dataCorrect)
    {
        const int a1 = 1;
        const int a2 = 9;									//	 尺寸预测 + 观测尺寸 加权得到预测尺寸
